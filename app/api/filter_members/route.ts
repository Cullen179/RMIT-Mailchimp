...
export async function GET(req: Request) {
  const authorization = headers().get("Authorization") as string;
    ...
    const memberAdd = headers().get("Members")?.split(',')
    .map(member => member.trim().toLowerCase())
    .filter(Boolean);

  let memberData: string[] = [];
  ...
  for (let i = 0; i < offset; i++) {
    ...
    // Get members not in the list
    const notMember = memberAdd?.filter((member) => !memberData.includes(member));
    const isMember = memberAdd?.filter((member) => memberData.includes(member));
